...
$rasistmetas = $mysqli->query($asistmetas);
while ($fasistmetas = $rasistmetas->fetch_assoc()) {
  // echo $fasistmetas['medida'];
  // echo "<br>";
  $count++;
  ...
  // echo $fasistmetas['medida'];
  // echo "<br>";
  $folioexpediente = $fasistmetas['folioexpediente'];
  $idsujeto = $fasistmetas['id_persona'];
  $cadena = $fasistmetas['medida'];
$resultado = strrchr($cadena, ".");
if ($resultado !== false) {
    $resultado = substr($resultado, 1); // Eliminar el punto inicial
    // echo $resultado; // Salida: txt
}else {
  $resultado = $fasistmetas['medida'];
}
  // echo "<br>";
  $cadena = $resultado;
...
<tr>
  ...
  <td><?php   echo 'Informe_Exp_'.$dos_primeros_digitos.'/'.$añoexpediente.'-'.$texto.'-';
   ...
   <td><?php echo date("d/m/Y", strtotime($fasistmetas['date_ejecucion'])); ?></td>
</tr>
<?php
}

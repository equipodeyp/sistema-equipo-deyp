<?php
$contador = 0;
$consulta1 = "SELECT solicitud_asistencia.*
FROM solicitud_asistencia
INNER JOIN (
SELECT DISTINCT id_asistencia
FROM cita_asistencia
WHERE fecha_asistencia BETWEEN '2026-01-01' AND '2026-12-31'
) AS citas_unicas
ON solicitud_asistencia.id_asistencia = citas_unicas.id_asistencia;";
...
while ($var_fila1=$var_resultado1->fetch_array())
{
  ...
  $var_fila5=$var_resultado5->fetch_array();
    ...
    echo "<td style='text-align:center; border: 1px solid black;'>"; if ($var_fila1['etapa'] === 'CANCELADA') {
      ...
      echo $var_fila4['traslado_realizado'];
    } echo "</td>";
    echo "<td style='text-align:center; border: 1px solid black;'>"; if ($var_fila1['etapa'] === 'CANCELADA') {
      echo "";
    }else {

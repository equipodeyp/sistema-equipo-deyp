...
$tipo_consulta = strtoupper($_POST['tipo_consultapdf']);
$usuario = $_POST['usuariopdf'];
...
$nameuser = "SELECT * FROM usuarios WHERE usuario = '$usuario'";
...
if ($tipo_consulta === 'GLOBAL' && $actividad === 'TODAS') {
  include("search_global.php");
}
// BUSQUEDA GLOBAL Y SEGUN LA ACTIVIDAD SELECCIONADA
if ($tipo_consulta === 'GLOBAL' && $actividad !== 'TODAS') {
  include("search_global_actividad.php");
}
// BUSQUEDA POR USUARIO Y TODAS SUS ACTIVIDADES
if ($tipo_consulta === 'POR USUARIO' && $actividad === 'TODAS') {
  include("search_usuario.php");
}
// BUSQUEDA POR USUARIO Y SEGUN LA ACTIVIDAD SELECCIONADA
if ($tipo_consulta === 'POR USUARIO' && $actividad !== 'TODAS') {
  include("search_usuario_actividad.php");
}
// FIN DE BUSQUEDAS
$mpdf->WriteHtml($html);

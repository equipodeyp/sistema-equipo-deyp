...
if ($actividad === '2') {
  $atender = "SELECT COUNT(*) AS suma FROM react_actividad
                 WHERE id_subdireccion = 4 AND usuario = '$name' AND usuario = '$name' AND id_actividad = 2 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
  ...
  $html .='<tr>
           ...
           WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 2 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
...
while ($ffacilitarname = $rfacilitarname ->fetch_assoc()) {
  ...
  $faccontact = "SELECT COUNT(*) AS suma FROM react_actividad
                 WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 3 AND clasificacion = '$varclasif' AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
  $rfaccontact = $mysqli->query($faccontact);
  $ffaccontact = $rfaccontact ->fetch_assoc();
  if ($aux){
    $html .='<tr>
              <td rowspan="5" width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> 1 </td>
...
$totalactividades = "SELECT COUNT(*) AS suma FROM react_actividad
WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 3 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
$rtotalactividades = $mysqli->query($totalactividades);
$ftotalactividades = $rtotalactividades ->fetch_assoc();
$html .='<tfoot>
          <tr>
...
elseif ($actividad === '6') {
  $recorridos = "SELECT COUNT(*) AS suma FROM react_actividad
                 WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 6 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
...
elseif ($actividad === '7') {
  $actividadessujetos = "SELECT COUNT(*) AS suma FROM react_actividad
                 WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 7 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
  $ractividadessujetos = $mysqli->query($actividadessujetos);
  $factividadessujetos = $ractividadessujetos ->fetch_assoc();
  $html .='<tr>
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> 1 </td>
            ...
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> '.$factividadessujetos['suma'].' </td>
           </tr>';
           $totalactividades = "SELECT COUNT(*) AS suma FROM react_actividad
           WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 7 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
           $rtotalactividades = $mysqli->query($totalactividades);
...
elseif ($actividad === '8') {
  $otrasactividades = "SELECT COUNT(*) AS suma FROM react_actividad
                 WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 8 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
  $rotrasactividades = $mysqli->query($otrasactividades);
  $fotrasactividades = $rotrasactividades ->fetch_assoc();
  $html .='<tr>
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> 1 </td>
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> OTRAS </td>
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> N/A </td>
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> RONDÍN POLICIAL </td>
            <td width="5%" style="border: 1px solid black; padding: 2px; text-align: center;"> '.$fotrasactividades['suma'].' </td>
           </tr>';
           $totalactividades = "SELECT COUNT(*) AS suma FROM react_actividad
           WHERE id_subdireccion = 4 AND usuario = '$name' AND id_actividad = 8 AND (fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')";
           $rtotalactividades = $mysqli->query($totalactividades);
...
$nombreact = "SELECT * FROM react_actividad_ejecucion WHERE id = '$actividad'";
$rnombreact = $mysqli->query($nombreact);
$fnombreact = $rnombreact->fetch_assoc();
        $html .='<table width="70%" style="border-spacing: 0;" bgcolor="#5F6D6B" align="left">
                  <thead class="thead-dark">
                    ...
                    <tr>
                      ...
                      <th style="background-color: #f0f0f0; border: 1px solid #A19E9F; text-align:center; font-family: gothambook;"><p style="text-align:center; color:black;"><font size=2>'.$fnombreact['nombre'].'</p></font></th>
                    </tr>
                    <tr>
